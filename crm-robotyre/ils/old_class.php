...
class parserRobotyre{
    ...
    private function requerst($type, $method, $param = '', $return){
       // $url = 'https://crm.robotyre.ru/api/' . $method . '/?';
        ...
        if($method == 'GetProducts') {
            $url = 'https://crm.robotyre.ru/api/GetProducts?token='.$this->getKey().'&date='.date('d.m.Y H:i:s', strtotime('now -2 hour'));
        } else {
            $url = 'https://crm.robotyre.ru/api/' . $method . '/?';
        }
    ...
    private function getJson(){
        ...
        if($res["message"] == 'Запрос устарел' || $res["message"] == 'id пуст'){
            $this->dropZaprosId();
            $this->createZapros();
            $this->getJson();
        }
